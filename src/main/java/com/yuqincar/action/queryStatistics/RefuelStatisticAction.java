...
@Controller
@Scope("prototype")
public class RefuelStatisticAction extends BaseAction {
	...
	public String list() throws Exception {
		...
		if(date!=null && !"".equals(date))
			helper.addWhereCondition("? <= cr.date and cr.date <= ?",DateUtils.getMinDate(date),DateUtils.getMaxDate(date));
	...
	public String weekList() throws Exception{
		...
		if(date!=null && !"".equals(date))
			helper.addWhereCondition("? <= cr.date and cr.date <= ?",DateUtils.getWeek(date).getLowDate(),DateUtils.getWeek(date).getHighDate());
	...
	public String monthList() throws Exception{
		...
		if(date!=null && !"".equals(date))
			helper.addWhereCondition("? <= cr.date and cr.date <= ?", DateUtils.getFirstDateOfMonth(date),DateUtils.getEndDateOfMonth(date));
	...
	public String query() throws Exception{
		...
		if(date1!=null && date2!=null)
			...
			helper.addWhereCondition("(TO_DAYS(cr.date)-TO_DAYS(?))>=0 and (TO_DAYS(?)-TO_DAYS(cr.date))>=0", date1 ,date2);
	...
	public void MoMHistogram() throws Exception{
			...
			for(int i=0;i<12;i++){
				...
				BigDecimal money=carRefuelService.statisticCarRefuel(DateUtils.getMinDate(begin),DateUtils.getMaxDate(end));
				dataset.addValue(money, "加油金额", x + "年" + (i+1) + "月");
	            lineDataset.addValue(money, "加油金额", x + "年" + (i+1) + "月");
			}
			
			JFreeChart chart = ChartFactory.createBarChart("加油金额(元)",
			     "",// 目录轴的显示标签
			     "", // 数值轴的显示标签
			     dataset,// 数据集
	...
	public void YoYHistogram() throws Exception{
			...
			if(x<=z){
				...
				Date end = DateUtils.getEndDateOfMonth(begin);
				BigDecimal money = carRefuelService.statisticCarRefuel(DateUtils.getMinDate(begin),DateUtils.getMaxDate(end));
				dataset.addValue(money, "加油金额", z + "年" + y + "月");
	            lineDataset.addValue(money, "加油金额", z + "年" + y + "月");
			}else{
				for(int i=z;i<=x;i++){
					Calendar calendar=Calendar.getInstance();
					...
					//System.out.println("end="+DateUtils.getYMDString(end));
					BigDecimal money=carRefuelService.statisticCarRefuel(DateUtils.getMinDate(begin),DateUtils.getMaxDate(end));
					dataset.addValue(money, "加油金额", i + "年" + y + "月");
		            lineDataset.addValue(money, "加油金额", i + "年" + y + "月");
				}
			}
