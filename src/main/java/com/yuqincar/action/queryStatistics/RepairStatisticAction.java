...
@Controller
@Scope("prototype")
public class RepairStatisticAction extends BaseAction {
	...
	public String list() throws Exception {
		...
		if(payDate!=null && !"".equals(payDate))
			helper.addWhereCondition("? <= cr.payDate and cr.payDate <= ?",DateUtils.getMinDate(payDate),DateUtils.getMaxDate(payDate));
	...
	public String weekList() throws Exception{
		...
		if(payDate!=null && !"".equals(payDate))
			helper.addWhereCondition("? <= cr.payDate and cr.payDate <= ?",DateUtils.getWeek(payDate).getLowDate(),DateUtils.getWeek(payDate).getHighDate());
	...
	public String monthList() throws Exception{
		...
		if(payDate!=null && !"".equals(payDate))
			helper.addWhereCondition("? <= cr.payDate and cr.payDate <= ?", DateUtils.getFirstDateOfMonth(payDate),DateUtils.getEndDateOfMonth(payDate));
	...
	public String query() throws Exception{
		...
		if(date1!=null && date2!=null)
			helper.addWhereCondition("(TO_DAYS(cr.payDate)-TO_DAYS(?))>=0 and (TO_DAYS(?)-TO_DAYS(cr.payDate))>=0", date1 ,date2);
		
		if(plateNumber!=null && !"".equals(plateNumber))
			helper.addWhereCondition("cr.car.plateNumber = ?", plateNumber );
		
		if(driverName!=null && !"".equals(driverName))
			helper.addWhereCondition("cr.car.driver.name like ?", "%" + driverName + "%");
	...
	public void MoMHistogram() throws Exception{
			...
			for(int i=0;i<12;i++){
				...
				BigDecimal money=carRepairService.statisticCarRepair(DateUtils.getMinDate(begin),DateUtils.getMaxDate(end));
				dataset.addValue(money, "维修金额", x + "年" + (i+1) + "月");
	            lineDataset.addValue(money, "维修金额", x + "年" + (i+1) + "月");
			}
			
			JFreeChart chart = ChartFactory.createBarChart("维修金额(元)",
			     "",// 目录轴的显示标签
			     "", // 数值轴的显示标签
			     dataset,// 数据集
	...
	public void YoYHistogram() throws Exception{
			...
			if(x<=z){
				...
				Date end = DateUtils.getEndDateOfMonth(begin);
				BigDecimal money = carRepairService.statisticCarRepair(DateUtils.getMinDate(begin), DateUtils.getMaxDate(end));
				dataset.addValue(money, "维修金额", z + "年" + y + "月");
	            lineDataset.addValue(money, "维修金额", z + "年" + y + "月");
			}else{
				for(int i=z;i<=x;i++){
					Calendar calendar=Calendar.getInstance();
					...
					//System.out.println("end="+DateUtils.getYMDString(end));
					BigDecimal money=carRepairService.statisticCarRepair(DateUtils.getMinDate(begin),DateUtils.getMaxDate(end));
					dataset.addValue(money, "维修金额", i + "年" + y + "月");
		            lineDataset.addValue(money, "维修金额", i + "年" + y + "月");
				}
			}
